import { Ionicons, FontAwesome5 } from "react-native-vector-icons";

export const MESSAGE_LIST_DATA = [
  {
    profile_pic:
      "https://e0.pxfuel.com/wallpapers/105/23/desktop-wallpaper-compromised-character-gaming-profile-dark-cute-cartoon-boys-thumbnail.jpg",
    name: "John Doe",
    message: "Hello, how are you?",
    time_sent: "2024-08-07T10:00:00Z",
    phone_number: "+1234567890",
  },
  {
    profile_pic:
      "https://e0.pxfuel.com/wallpapers/105/23/desktop-wallpaper-compromised-character-gaming-profile-dark-cute-cartoon-boys-thumbnail.jpg",
    name: "Jane Smith",
    message: "Good morning!",
    time_sent: "2024-08-07T10:15:00Z",
    phone_number: "+1234567891",
  },
  {
    profile_pic:
      "https://e0.pxfuel.com/wallpapers/105/23/desktop-wallpaper-compromised-character-gaming-profile-dark-cute-cartoon-boys-thumbnail.jpg",
    name: "Alice Johnson",
    message: "Meeting at 3 PM.",
    time_sent: "2024-08-07T10:30:00Z",
    phone_number: "+1234567892",
  },
  {
    profile_pic:
      "https://e0.pxfuel.com/wallpapers/105/23/desktop-wallpaper-compromised-character-gaming-profile-dark-cute-cartoon-boys-thumbnail.jpg",
    name: "Bob Brown",
    message: "Can you send me the report?",
    time_sent: "2024-08-07T10:45:00Z",
    phone_number: "+1234567893",
  },
  {
    profile_pic:
      "https://e0.pxfuel.com/wallpapers/105/23/desktop-wallpaper-compromised-character-gaming-profile-dark-cute-cartoon-boys-thumbnail.jpg",
    name: "Charlie Davis",
    message: "Let's grab lunch.",
    time_sent: "2024-08-07T11:00:00Z",
    phone_number: "+1234567894",
  },
  {
    profile_pic:
      "https://e0.pxfuel.com/wallpapers/105/23/desktop-wallpaper-compromised-character-gaming-profile-dark-cute-cartoon-boys-thumbnail.jpg",
    name: "Diana Evans",
    message: "Don't forget the meeting.",
    time_sent: "2024-08-07T11:15:00Z",
    phone_number: "+1234567895",
  },
  {
    profile_pic:
      "https://e0.pxfuel.com/wallpapers/105/23/desktop-wallpaper-compromised-character-gaming-profile-dark-cute-cartoon-boys-thumbnail.jpg",
    name: "Ethan Green",
    message: "How was your weekend?",
    time_sent: "2024-08-07T11:30:00Z",
    phone_number: "+1234567896",
  },
  {
    profile_pic:
      "https://e0.pxfuel.com/wallpapers/105/23/desktop-wallpaper-compromised-character-gaming-profile-dark-cute-cartoon-boys-thumbnail.jpg",
    name: "Fiona Harris",
    message: "See you at 2 PM.",
    time_sent: "2024-08-07T11:45:00Z",
    phone_number: "+1234567897",
  },
  {
    profile_pic:
      "https://e0.pxfuel.com/wallpapers/105/23/desktop-wallpaper-compromised-character-gaming-profile-dark-cute-cartoon-boys-thumbnail.jpg",
    name: "George Ivanov",
    message: "Can you call me later?",
    time_sent: "2024-08-07T12:00:00Z",
    phone_number: "+1234567898",
  },
  {
    profile_pic:
      "https://e0.pxfuel.com/wallpapers/105/23/desktop-wallpaper-compromised-character-gaming-profile-dark-cute-cartoon-boys-thumbnail.jpg",
    name: "Hannah Jones",
    message: "Please review the attached document.",
    time_sent: "2024-08-07T12:15:00Z",
    phone_number: "+1234567899",
  },
];

export const APP_PAGES = {
  APP: {
    SPEC_CHAT: "SPEC_CHAT",
    HOME: "HOME",
  },
};

export const APP_ICONS = {
  BACK: (
    <Ionicons
      name={"arrow-back-outline"}
      style={{ color: "#fff", fontSize: 20 }}
    />
  ),
  SAVE: (
    <Ionicons
      name={"checkmark-outline"}
      style={{ color: "#fff", fontSize: 20 }}
    />
  ),
  PHONE: (
    <Ionicons name={"call-outline"} style={{ color: "#fff", fontSize: 20 }} />
  ),
  MIC: (
    <Ionicons name={"mic-outline"} style={{ color: "#fff", fontSize: 20 }} />
  ),
  SEARCH: (
    <Ionicons name={"search-outline"} style={{ color: "#fff", fontSize: 20 }} />
  ),
  VIDEO: (
    <Ionicons
      name={"videocam-outline"}
      style={{ color: "#fff", fontSize: 20 }}
    />
  ),
  DOTS: (
    <Ionicons
      name={"ellipsis-vertical-outline"}
      style={{ color: "#fff", fontSize: 20 }}
    />
  ),
  DOC: (
    <Ionicons name={"attach-outline"} style={{ color: "#fff", fontSize: 20 }} />
  ),
  CAMERA: (
    <Ionicons name={"camera-outline"} style={{ color: "#fff", fontSize: 20 }} />
  ),
  IMJIE: (
    <Ionicons name={"happy-outline"} style={{ color: "#fff", fontSize: 20 }} />
  ),
  READ: (
    <Ionicons
      name={"checkmark-done-outline"}
      style={{ color: "gray", fontSize: 16 }}
    />
  ),
  SENT: (
    <Ionicons
      name={"checkmark-outline"}
      style={{ color: "gray", fontSize: 16 }}
    />
  ),
  SEARCH: (
    <Ionicons name={"search-outline"} style={{ color: "#fff", fontSize: 20 }} />
  ),
};
